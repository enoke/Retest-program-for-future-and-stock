...
class collectdata():
    def __init__(self, txt="Crypto.txt", strategy=" ", d_entry=False, A_period=" ", B_period=" ", exit_mode="",ma_num=20,percent=0.02,gap_week=0):
        self.address_dir="D:/AIW20241215/AIW20241215/AIW"
        self.DK_folder="D:/AIW20241215/AIW20241215/AIW/data_S/DK"
        self.WK_folder="D:/AIW20241215/AIW20241215/AIW/data_S/WK"
        self.MK_folder="D:/AIW20241215/AIW20241215/AIW/data_S/MK"
        self.TWII_wk_csv="D:/AIW20241215/AIW20241215/AIW/data_S/WK/^TWII.csv"
        self.txt=txt
    ...
    def batch_backtest_sma_strategy(self, export_dir='SMA'):
        ...
        for filename in self.stock_id:
            ...
            if export_dir=="RSI":
                ...
                trades_df = collectdata.backtest_sma_strategy_V8(weekly_csv, daily_csv, show_summary=False)
                #trades_df = collectdata.backtest_sma_strategy_V3G(weekly_csv, daily_csv, signal_tf="month", ma_days=20, show_summary=False, direct_entry_no_retest=self.d_entry)
                #trades_df = collectdata.backtest_sma_strategy_V8(weekly_csv, daily_csv, signal_tf=self.A_period, retest_tf=self.B_period, ma_days=20, tp_pct=0.035, monthly_csv=monthly_csv, direct_entry_no_retest=self.d_entry)
            elif export_dir=="VA":
                ...
                trades_df, levels_df = collectdata.backtest_candle_turn_strategy_v6RG(weekly_csv,daily_csv, signal_tf=self.A_period,max_gap_weeks=self.gap_week, retest_tf=self.B_period,tp_pct=self.percent,exit_mode=self.exit_mode,exit_ma_days=self.ma_num, monthly_csv=monthly_csv, direct_entry_no_retest=self.d_entry)
                # 🔸新增：收集轉折水平表
    ...
    def backtest_sma_strategy_V3G(
        ...
        *,
        # ★ 新增：出場模式選擇
        exit_mode: str = 'competing',  # 'sma5', 'pct', 或 'competing'
        tp_pct: float | None = 0.01, # (當 exit_mode='pct' 或 'competing' 時生效)
        sl_pct: float | None = None  # (當 exit_mode='pct' 或 'competing' 時生效)
        ):
        """
        ★ MODIFIED: V3_selectable_exit (基於 V3_mod2)
        
        進場條件（同 V3_mod2）：
        (週 Close 上穿/下穿 週SMA5) -> 下一週日線出現 (A) 或 (B) 模式 (取最早者)
        (A) 單日回測：(多) Low<=20MA 且 Close>20MA / (空) High>=20MA 且 Close<20MA
        (B) 兩日回測：(多) Day1 Low<=20MA, Day2 Close>20MA / (空) Day1 High>=20MA, Day2 Close<20MA
        → 觸T發日『下一個交易日開盤』進場

        出場條件（★ 可選模式 ★）：
        - exit_mode='sma5': (預設) 第一次出現 週 Close 跌破/突破 週SMA5 的那一週收盤。
        - exit_mode='pct':  日收盤價觸及 tp_pct 或 sl_pct。
        - exit_mode='competing': 'sma5' 和 'pct' 競爭，以最早發生的為準。
        """
        ...
        wk['prev_Close'] = wk['Close'].shift(1)
        wk['prev_Sma5']  = wk['Sma5'].shift(1)
        wk['bull_cross'] = (wk['prev_Close'] < wk['prev_Sma5']) & (wk['Close'] > wk['Sma5'])
        wk['bear_cross'] = (wk['prev_Close'] > wk['prev_Sma5']) & (wk['Close'] < wk['Sma5'])
        dk['SMA20']      = dk['Close'].rolling(20, min_periods=20).mean()

        trades = []
        norm_exit_mode = (exit_mode or 'sma5').strip().lower()
        if norm_exit_mode not in ('sma5', 'pct', 'competing'):
            raise ValueError(f"exit_mode 必須是 'sma5', 'pct', 或 'competing'，但收到了: {exit_mode}")
        if norm_exit_mode == 'pct' and (tp_pct is None and sl_pct is None):
            raise ValueError("當 exit_mode='pct' 時, 至少必須提供 tp_pct 或 sl_pct")

        # --- (進場邏輯 _find_entry_in_next_week 同 V3_mod2 保持不變) ---
        def _find_entry_in_next_week(signal_week_idx: int, side: str):
            if signal_week_idx + 1 >= len(wk): return None
            week_signal_date = wk.loc[signal_week_idx, 'Date']
            next_week_end    = wk.loc[signal_week_idx + 1, 'Date']
            next_week_start  = week_signal_date
            dw = dk[(dk['Date'] > next_week_start) & (dk['Date'] <= next_week_end)].copy()
            if dw.empty: return None
            trigger_date_1day = pd.NaT; trigger_date_2day = pd.NaT
            if side == 'long':
                cond_1day = (dw['Low'] <= dw['SMA20']) & (dw['Close'] > dw['SMA20'])
            else:
                cond_1day = (dw['High'] >= dw['SMA20']) & (dw['Close'] < dw['SMA20'])
            hits_1day = dw[cond_1day]
            if not hits_1day.empty:
                trigger_date_1day = hits_1day.iloc[0]['Date']
            if len(dw) >= 2:
                for i in range(len(dw) - 1):
                    d1 = dw.iloc[i]; d2 = dw.iloc[i+1]
                    sma1 = d1['SMA20']; sma2 = d2['SMA20']
                    if pd.isna(sma1) or pd.isna(sma2): continue
                    if side == 'long':
                        cond_d1 = (d1['Low'] <= sma1); cond_d2 = (d2['Close'] > sma2)
                    else:
                        cond_d1 = (d1['High'] >= sma1); cond_d2 = (d2['Close'] < sma2)
                    if cond_d1 and cond_d2:
                        trigger_date_2day = d2['Date']; break
            trigger_date = pd.NaT; trigger_type = None
            if pd.notna(trigger_date_1day) and pd.notna(trigger_date_2day):
                if trigger_date_1day <= trigger_date_2day:
                    trigger_date = trigger_date_1day; trigger_type = '1day_touch_close'
                else:
                    trigger_date = trigger_date_2day; trigger_type = '2day_retest'
            elif pd.notna(trigger_date_1day):
                trigger_date = trigger_date_1day; trigger_type = '1day_touch_close'
            elif pd.notna(trigger_date_2day):
                trigger_date = trigger_date_2day; trigger_type = '2day_retest'
            else:
                return None
        ...
        # --- (出場邏輯 1: SMA5 週線出場) ---
        def _find_exit_by_sma5(signal_week_date: pd.Timestamp, side: str):
            future_wk = wk[wk['Date'] > signal_week_date].copy()
            if future_wk.empty: return None
            if side == 'long':
            ...
            if not future_wk[cond_exit].empty:
                ...
                reason = 'SMA5_wk_cross'
            else:
                exit_row = future_wk.iloc[-1] # 強制出場
                reason = 'FORCED_LAST_WEEK'
            return (exit_row['Date'], float(exit_row['Close']), reason)

        # --- (出場邏輯 2: 日線 % 出場) ---
        # ★ MODIFIED: _find_exit_by_pct
        # 即使未觸發 TP/SL，也必須在最後一天強制出場 (如果 'pct' 是唯一模式)
        def _find_exit_by_pct(entry_date, entry_price, side, tp_pct, sl_pct):
            if tp_pct is None and sl_pct is None:
                return None # (安全檢查，理論上在主流程已擋掉)
            future_days = dk[dk['Date'] > entry_date].copy()
            if future_days.empty: return None

            target_tp = None
            if tp_pct is not None:
                tp_pct_abs = abs(float(tp_pct))
                target_tp = entry_price * (1.0 + tp_pct_abs) if side == 'long' else entry_price * (1.0 - tp_pct_abs)
            target_sl = None
            if sl_pct is not None:
                sl_pct_abs = abs(float(sl_pct)) 
                target_sl = entry_price * (1.0 - sl_pct_abs) if side == 'long' else entry_price * (1.0 + sl_pct_abs)

            for _, r in future_days.iterrows():
                c = float(r['Close'])
                if side == 'long':
                    if target_sl is not None and c <= target_sl:
                        return (r['Date'], c, 'SL_pct')
                    if target_tp is not None and c >= target_tp:
            ...
            # ★ NEW: 如果 'pct' 是唯一模式，不能返回 None，必須強制出場
            if not future_days.empty:
                 r_last = future_days.iloc[-1]
                 return (r_last['Date'], float(r_last['Close']), 'FORCED_LAST_DAY')
            
            return None # 真的沒有未來數據了

        # ★ MODIFIED: ── 多頭流程 (依 exit_mode 選擇出場) ─────────────
        for idx in wk.index[wk['bull_cross']].tolist():
            entry_pack = _find_entry_in_next_week(idx, side='long')
            if entry_pack is None: continue
            entry_date, entry_price, trigger_date, trigger_type = entry_pack

            exit_date, exit_price, exit_reason = pd.NaT, np.nan, None
            
            # 獲取兩種可能的出場 (如果需要)
            sma5_exit_pack = None
            if norm_exit_mode in ('sma5', 'competing'):
                sma5_exit_pack = _find_exit_by_sma5(wk.loc[idx, 'Date'], side='long')
            
            pct_exit_pack = None
            if norm_exit_mode in ('pct', 'competing'):
                pct_exit_pack = _find_exit_by_pct(entry_date, entry_price, 'long', tp_pct, sl_pct)

            # 根據模式決定最終出場
            if norm_exit_mode == 'sma5':
                if sma5_exit_pack:
                    exit_date, exit_price, exit_reason = sma5_exit_pack
                else: continue # 找不到出場
            
            elif norm_exit_mode == 'pct':
                if pct_exit_pack:
                    exit_date, exit_price, exit_reason = pct_exit_pack
                else: continue # 找不到出場 (例如沒有未來日線)

            elif norm_exit_mode == 'competing':
                if sma5_exit_pack is None or pct_exit_pack is None:
                    continue # 缺少必要數據
                
                if pct_exit_pack[0] <= sma5_exit_pack[0]: # % 出場日 <= SMA5 出場日
                    exit_date, exit_price, exit_reason = pct_exit_pack
                else:
                    exit_date, exit_price, exit_reason = sma5_exit_pack

            if pd.isna(exit_date): continue # 最終檢查
        ...
        # ★ MODIFIED: ── 空頭流程 (依 exit_mode 選擇出場) ─────────────
        for idx in wk.index[wk['bear_cross']].tolist():
            ...
            sma5_exit_pack = None
            if norm_exit_mode in ('sma5', 'competing'):
                sma5_exit_pack = _find_exit_by_sma5(wk.loc[idx, 'Date'], side='short')
            
            pct_exit_pack = None
            if norm_exit_mode in ('pct', 'competing'):
                pct_exit_pack = _find_exit_by_pct(entry_date, entry_price, 'short', tp_pct, sl_pct)
            ...
            if norm_exit_mode == 'sma5':
                ...
                if sma5_exit_pack is None or pct_exit_pack is None:
                    continue
                if pct_exit_pack[0] <= sma5_exit_pack[0]:
                    exit_date, exit_price, exit_reason = pct_exit_pack
                else:
                    exit_date, exit_price, exit_reason = sma5_exit_pack

            if pd.isna(exit_date): continue
        ...
        if not trades_df.empty:
            ...
            if show_summary:
                n   = len(trades_df)
                wr = (trades_df['pnl'] > 0).mean() if n > 0 else 0
                tot = (1 + trades_df['pnl_pct']/100).prod() - 1 if n > 0 else 0
                print(f"[{symbol if symbol else 'SMA_V3_Selectable'}] ExitMode: {norm_exit_mode}, Trades: {n}, WinRate: {wr:.2%}, TotalRet: {tot:.2%}  (long={sum(trades_df['direction']=='long')}, short={sum(trades_df['direction']=='short')})")
    ...
    def backtest_candle_turn_strategy_v6RG(
                weekly_csv,
                daily_csv,
                *,
                symbol=None,
                tp_pct: float = 0.03,
                retest_tol: float = 0.0,
                show_summary: bool = False,
                direct_entry_no_retest: bool = True,
                signal_tf: str = "week",
                retest_tf: str = "day", 
                ...
                # ★ NEW PARAM:
                retest_window_weeks: int | None = None, # ★ 新增：若設定此值（如 4），則啟用「轉折後N週內日線回測」新邏輯
            ):
            """
            === V6 原始邏輯 (當 retest_window_weeks=None) ===
            出場條件（擇一）：
            - exit_mode='tp_pct' ：價格相對進場價達到 tp_pct 就出場
            - exit_mode='ma'     ：日線對 SMA(exit_ma_days) 發生反向交叉
            其他邏輯：
            - 以 signal_tf（週/月）偵測『顏色翻轉』求轉折價 L
            - 找到首次「收盤突破/跌破 L」的訊號期 → 僅在「下一個訊號期」內用 retest_tf（日/週）找觸發
            - direct_entry_no_retest=True：不回測，於下一期第一根（retest_tf 粒度）收盤進場
            - 以 max_gap_weeks 篩選：若 轉折→突破 之間 gap_weeks > max_gap_weeks，忽略

            === V6_Mod (v7) 新邏輯 (當 retest_window_weeks=N) ===
            - 以 signal_tf（週/月）偵測『顏色翻轉』求轉折價 L
            - ★ 邏輯變更：不再等待 signal_tf 突破
            - ★ 而是從轉折K棒結束後起算 retest_window_weeks（如 4 週）
            - ★ 在此 N 週的「日線」中，尋找「回測 L 且反彈/失敗」的訊號
            -   - (轉折低點 L -> 支撐 -> 找日線回測 L 且反彈 -> 做多 Long)
            -   - (轉折高點 L -> 壓力 -> 找日線回測 L 且失敗 -> 放空 Short)
            - ★ 此模式下，retest_tf 強制為 'day'
            - ★ 此模式下，direct_entry_no_retest 和 max_gap_weeks 參數失效
            - 出場邏輯 (tp_pct / ma) 不變
            """
            import pandas as pd
            import numpy as np
            ...
            def _find_candle_turns(df: pd.DataFrame) -> pd.DataFrame:
                ...
                for i in range(1, len(df)):
                    ...
                    if prev_up and curr_down:
                        L = float(max(prev['Close'], curr['Open']))
                        turns.append({'date': curr['Date'], 'type': 'high', 'price': L}) # 壓力
                    if prev_down and curr_up:
                        L = float(min(prev['Close'], curr['Open']))
                        turns.append({'date': curr['Date'], 'type': 'low',  'price': L}) # 支撐
                return pd.DataFrame(turns, columns=['date','type','price']) if len(turns) else pd.DataFrame(columns=['date','type','price'])
            ...
            # ★ MODIFIED: 根據 retest_window_weeks 決定執行哪個邏輯
            is_new_logic = (retest_window_weeks is not None) and (int(retest_window_weeks) > 0)
            
            # ★★★ A. NEW LOGIC (v7) ★★★
            if is_new_logic:
                if show_summary:
                    print(f"[{sym}] Running NEW logic: Retest within {retest_window_weeks} weeks.")
                ...
                for _, t in turns.iterrows():
                    ...
                    # ★ NEW: 交易方向與 V6 相反。回測支撐 -> 做多；回測壓力 -> 放空
                    side = 'short' if turn_type == 'high' else 'long'
                    
                    # ★ NEW: 定義 N 週的回測搜索窗口
                    search_start_date = turn_period_end
                    search_end_date   = turn_period_end + pd.Timedelta(weeks=int(retest_window_weeks))
                    
                    # ★ NEW: 取得此窗口內的「日線」數據
                    window = dk[
                        (dk['Date'] > search_start_date) & 
                        (dk['Date'] <= search_end_date)
                    ].copy()

                    if window.empty:
                        continue
                        
                    # ★ NEW: 在日線窗口中，應用 A/B 回測邏輯
                    # (此模式下 direct_entry_no_retest=True 失效，強制要求回測)
                    entry_date = None
                    entry_price = None
                    trigger_date = None
                    retest_mode = None
                    ...
                    if side == 'long': # 回測支撐 L
                        # A: 觸碰支撐 (tol_up) 且反彈 (綠K, 收盤 > L)
                        condA = (window['Low']  <= L * tol_up) & (window['Close'] > window['Open']) & (window['Close'] > L)
                    else: # side == 'short' (回測壓力 L)
                        # A: 觸碰壓力 (tol_dn) 且失敗 (紅K, 收盤 < L)
                        condA = (window['High'] >= L * tol_dn) & (window['Close'] < window['Open']) & (window['Close'] < L)
                    ...
                    # ★ NEW: 計算 轉折 -> 觸發 的天數
                    gap_days = (trigger_date - turn_period_end).days
                    ...
                    trades.append({
                        ...
                        'breakout_week': pd.NaT, # 新邏輯無此項
                        'gap_weeks_from_turn_to_breakout': None, # 新邏輯無此項
                        'gap_days_from_turn_to_trigger': int(gap_days), # ★ NEW
                        'retest_window_weeks': int(retest_window_weeks), # ★ NEW
                        
                        'retest_tf': 'day', # 新邏輯強制 'day'
                        'retest_mode': retest_mode,
                        'trigger_date': trigger_date,
                        'direction': side,
                        'entry_date': entry_date,
            ...
            # ★★★ B. ORIGINAL LOGIC (v6) ★★★
                ...
                if show_summary:
                    print(f"[{sym}] Running ORIGINAL V6 logic: Breakout-first.")
                ...
                for _, t in turns.iterrows():
                    ...
                    if direct_entry_no_retest:
                        ...
                        # (V6 的 A/B/W 回測邏輯)
                        entry_date = None; entry_price = None; trigger_date = None
                        if norm_retest_tf == 'day':
                            if side == 'long':
                                condA = (window['Low']  <= L * tol_up) & (window['Close'] > window['Open']) & (window['Close'] > L)
                            else:
                    ...
                    trades.append({
                        ...
                        'breakout_week': breakout_period_end,
                        'gap_weeks_from_turn_to_breakout': int(gap_weeks),
                        'gap_days_from_turn_to_trigger': None, # V6 邏輯無此項
                        'retest_window_weeks': None, # V6 邏輯無此項
            ...
            # === ★ MODIFIED: gap 的統計摘要 (根據 is_new_logic 決定統計欄位) ===
            summary_gap_stats = {}
            summary_gap_dist = {}
            summary_gap_by_win = {}
            
            if is_new_logic:
                gap_col = 'gap_days_from_turn_to_trigger'
                gap_unit = 'days'
                def _bucket_days(x: int):
                    x = int(x)
                    if x <= 7: return '0-7d'
                    if x <= 14: return '8-14d'
                    if x <= 21: return '15-21d'
                    return '22d+'
                _bucket_func = _bucket_days
            else:
                gap_col = 'gap_weeks_from_turn_to_breakout'
                gap_unit = 'weeks'
                def _bucket_weeks(x: int):
                    x = int(x)
                    if x <= 2: return '0-2w'
                    if x <= 4: return '3-4w'
                    if x <= 8: return '5-8w'
                    return '9w+'
                _bucket_func = _bucket_weeks
            ...
            try:
                ...
                if len(g) > 0:
                    ...
                    summary_gap_stats = {'count': 0, 'min': None, 'max': None, 'mean': None, 'median': None}
            except Exception:
                summary_gap_stats = {'count': 0, 'min': None, 'max': None, 'mean': None, 'median': None}

            try:
                summary_gap_dist = trades_df[gap_col].dropna().astype(int).map(_bucket_func).value_counts().to_dict()
            except Exception:
                summary_gap_dist = {}
                
            try:
                summary_gap_by_win = trades_df.assign(win=(trades_df['pnl']>0)).groupby('win')[gap_col].mean().to_dict()
            except Exception:
                summary_gap_by_win = {}
            ...
            if show_summary:
                ...
                print(f"Logic Mode: {'Immediate_Retest' if is_new_logic else 'Breakout_Retest'}")
                mean_val = summary_gap_stats.get('mean')
                print(f"gap_{gap_unit}: count={summary_gap_stats.get('count', 0)}, min={summary_gap_stats.get('min')}, "
                    f"median={summary_gap_stats.get('median')}, mean={mean_val:.2f} if mean_val is not None else 'N/A', "
                    f"max={summary_gap_stats.get('max')}")
                if summary_gap_dist:
                    print(f"gap buckets: {summary_gap_dist}")
                if summary_gap_by_win:
                    print(f"avg gap by win/loss: {summary_gap_by_win}")

            # === ★ MODIFIED: 輸出 Excel (加入新欄位) ===
            if export_excel_path:
                summary_rows = [
                    {'metric': 'symbol', 'value': sym},
                    {'metric': 'trades', 'value': n_trades},
                    ...
                    {'metric': 'logic_mode', 'value': 'immediate_retest' if is_new_logic else 'breakout_retest'},
                    {'metric': 'retest_window_weeks_filter', 'value': retest_window_weeks}, # (was None for old logic)
                    {'metric': 'max_gap_weeks_filter', 'value': max_gap_weeks}, # (was None for new logic)
                    
                    {'metric': f'gap_{gap_unit}_count', 'value': summary_gap_stats.get('count')},
                    {'metric': f'gap_{gap_unit}_min', 'value': summary_gap_stats.get('min')},
                    {'metric': f'gap_{gap_unit}_median', 'value': summary_gap_stats.get('median')},
                    {'metric': f'gap_{gap_unit}_mean', 'value': summary_gap_stats.get('mean')},
                    {'metric': f'gap_{gap_unit}_max', 'value': summary_gap_stats.get('max')},
                ]
                for k, v in summary_gap_dist.items():
                    summary_rows.append({'metric': f'gap_bucket_{k}', 'value': v})
                for k, v in summary_gap_by_win.items():
                    label = 'win_true' if k is True else 'win_false'
                    summary_rows.append({'metric': f'avg_gap_{label}', 'value': v})
...
G_draw=False
G_sma_WeekDay=False
G_sma_WeekDay_sweep=True
G_VA_WeekDay=False
data=collectdata(txt="SP.txt", strategy="VA", d_entry=True)
if G_data==True:
    data.Get_data()
if G_D2W==True:
    data.D2W()
...
if G_sma_WeekDay==True:
    ...
    data.percent=0.02
    data.gap_week=20
    All_trades=data.batch_backtest_sma_strategy()
if G_sma_WeekDay_sweep==True:
     # 固定參數（不變）
    data.A_period = "week"
    data.B_period = "day"
    data.exit_mode = "tp_pct"
    ...
    pct_list = np.round(np.arange(pct_start, pct_end + 1e-12, pct_step), 6)

    results = []
    base_strategy = data.strategy  # 先記住原本的 strategy（例如 "VA"）

    for pct in pct_list:
        data.percent = float(pct)

        # 避免每次匯出互相覆蓋：用不同資料夾
        #data.strategy = f"{base_strategy}_pct_{pct:.4f}"
        data.strategy = "VA"

        all_trades = data.batch_backtest_sma_strategy()

        total_n = len(all_trades)
        win_rate = float((all_trades["pnl"] > 0).mean()) if total_n > 0 else 0.0
        total_pnl = float(all_trades["pnl_pct"].sum()) if total_n > 0 else 0.0
        ...
        print(f"[percent={pct:.4f}] 總筆數={total_n}, 勝率={win_rate:.2%}, 總損益(pnl_pct.sum)={total_pnl:.4f}")

    # 跑完後把 strategy 改回來
    data.strategy = base_strategy

    # 彙總表
    df = pd.DataFrame(results).sort_values("total_pnl_pct", ascending=False)
    print("\n=== percent sweep summary (sorted by total_pnl_pct) ===")
    print(df.to_string(index=False))

    # 也可選擇存檔（不想存就註解）
    sweep_csv_path = os.path.join(data.address_dir, 'Sweep.csv')
    df.to_csv(sweep_csv_path, index=False, encoding="utf-8-sig")
'''
# 設定股票代號
symbol = '2330.TW'  # 以台積電(2330)為例
# 使用yfinance套件抓取台股資料
